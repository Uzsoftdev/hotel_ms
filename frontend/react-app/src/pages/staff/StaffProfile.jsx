    ...
    <StaffLayout>
      <div className="max-w-2xl space-y-8">
        <div>
          ...
          <p className="text-sm text-on-surface-variant mt-1">Manage your staff account details</p>
        </div>
        ...
        <div className="rounded-xl border border-outline-variant/30 bg-surface-bright p-6 flex items-center gap-6">
          <div className="w-16 h-16 rounded-full bg-blue-100 flex items-center justify-center shrink-0">
            <span className="text-2xl font-extrabold text-blue-700">{initials || "S"}</span>
          </div>
          <div>
            <p className="text-lg font-extrabold text-on-surface">{profile.full_name || "Staff"}</p>
            <p className="text-sm text-on-surface-variant">{profile.email}</p>
            <span className="mt-1 inline-block text-[10px] font-bold uppercase tracking-widest bg-blue-100 text-blue-700 px-2 py-0.5 rounded">
              {user?.role || "Staff"}
            </span>
          </div>
        </div>
        ...
        <section className="rounded-xl border border-outline-variant/30 bg-surface-bright p-6 space-y-5">
          ...
          <form onSubmit={saveProfile} className="space-y-4">
            {[
              { label: "Full Name", key: "full_name", type: "text", placeholder: "Staff Member" },
              { label: "Email Address", key: "email", type: "email", placeholder: "staff@azurehorizon.com" },
