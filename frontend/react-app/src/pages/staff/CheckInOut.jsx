...
import { getAssignedBookings, checkInGuest, checkOutGuest } from "../../services/staff";
    ...
    <StaffLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-on-surface-variant mt-1">Process guest arrivals and departures</p>
        </div>
        {loading ? (
            ...
            {bookings.map((b) => (
              <div key={b.id} className="bg-white rounded-xl border border-outline-variant/30 p-6 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => handle(checkInGuest, b.id)} disabled={acting === b.id}
                    className="bg-green-600 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-green-700 disabled:opacity-50 flex items-center gap-1.5">
                    <span className="material-symbols-outlined text-[16px]">login</span>Check In
                  </button>
                  <button onClick={() => handle(checkOutGuest, b.id)} disabled={acting === b.id}
                    className="bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-blue-700 disabled:opacity-50 flex items-center gap-1.5">
