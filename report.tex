...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Name} & \textbf{Student ID} & \textbf{Section} & \textbf{Role} \\
\midrule
Axmadov Sunnat          & U2310044 & 002 & Team Lead $\cdot$ Backend Architecture \\
Mirshod Hamroyev        & U2310088 & 001 & Backend API \& Authentication \\
Nurmuhammad Fazliddinov & U2310082 & 001 & Database Design \& Migrations \\
Hamidov Abror           & U2310087 & 001 & DevOps \& Infrastructure \\
Yusufova Muxlisa        & U2310298 & 001 & Frontend \& UI Design \\
Izbosarov Bekzod        & U2310113 & 003 & Observability \& Testing \\
\bottomrule
\end{tabular}
...
The backend is built on \textbf{FastAPI} with a \textbf{PostgreSQL~15} primary database, a hot-standby streaming replica, and \textbf{PgBouncer} for connection pooling. \textbf{Redis} serves as an availability cache (with atomic invalidation on booking mutations), Celery task broker, and token-bucket state store. \textbf{Meilisearch} provides millisecond full-text hotel and room search with a PostgreSQL \texttt{ILIKE} fallback. Files (avatars, hotel images) are stored in \textbf{MinIO} (S3-compatible). Two FastAPI replicas sit behind a \textbf{Traefik~v3.1} API gateway with TLS termination and load balancing. A \textbf{Celery Beat} scheduler drives nightly revenue aggregation and hourly cache warm-up. A token-bucket rate limiter---implemented from scratch using Redis Lua scripts---protects all endpoints. Real-time booking notifications are delivered via \textbf{WebSocket} with Redis Pub/Sub cross-replica fan-out; the React frontend connects automatically on login and displays toast notifications. The entire stack is observable through \textbf{Prometheus}, \textbf{Grafana}, \textbf{Loki}, and \textbf{Jaeger} via OpenTelemetry.
...
\begin{lstlisting}[language=bash, numbers=none, frame=single, basicstyle=\ttfamily\tiny]
hotel_management_system/
|-- backend/
|   |-- app/
|   |   |-- api/v1/
|   |   |   |-- public/
|   |   |   |   |-- auth.py          # register, login, OAuth, verify, reset
|   |   |   |   `-- search.py        # hotel/room search + availability (R5,R6)
|   |   |   |-- user/
|   |   |   |   |-- bookings.py      # create, list, cancel booking
|   |   |   |   |-- notifications.py # notification list + mark-read
|   |   |   |   |-- payments.py      # payment history
|   |   |   |   |-- profile.py       # get/update profile, avatar upload
|   |   |   |   |-- reviews.py       # CRUD reviews
|   |   |   |   `-- wishlist.py      # save/remove hotels and rooms
|   |   |   |-- admin/
|   |   |   |   |-- blackout_dates.py
|   |   |   |   |-- bookings.py      # all bookings, check-in/out, status override
|   |   |   |   |-- hotels.py        # hotel CRUD + image management
|   |   |   |   |-- pricing.py       # dynamic pricing rules
|   |   |   |   |-- reports.py       # occupancy, revenue, guest analytics
|   |   |   |   |-- room_types.py
|   |   |   |   |-- rooms.py
|   |   |   |   `-- users.py         # user/staff management
|   |   |   `-- ws.py                # WebSocket endpoint /ws/{user_id} (R7)
|   |   |-- components/
|   |   |   `-- token_bucket.py      # Redis Lua token-bucket (R11)
|   |   |-- core/
|   |   |   |-- config.py            # Pydantic settings from .env
|   |   |   |-- database.py          # SQLAlchemy engine, SessionLocal, replica
|   |   |   |-- security.py          # JWT encode/decode, password hashing
|   |   |   `-- tracing.py           # OpenTelemetry + Jaeger setup (R12)
|   |   |-- middleware/
|   |   |   |-- auth.py              # DBSessionMiddleware
|   |   |   |-- observability.py     # PrometheusMiddleware, /metrics (R12)
|   |   |   |-- rbac.py              # role-based access control
|   |   |   `-- token_bucket_middleware.py  # ASGI rate-limit wrapper (R11)
|   |   |-- models/                  # SQLAlchemy ORM (20 tables) (R3)
|   |   |   |-- activity_log.py      |-- amenity.py
|   |   |   |-- blackout_date.py     |-- booking.py
|   |   |   |-- email_verification.py|-- facility.py
|   |   |   |-- hotel_facility.py    |-- hotel_image.py
|   |   |   |-- hotel.py             |-- notification.py
|   |   |   |-- password_reset.py    |-- payment.py
|   |   |   |-- pricing_rule.py      |-- review.py
|   |   |   |-- room_amenity.py      |-- room_image.py
|   |   |   |-- room_type.py         |-- room.py
|   |   |   |-- user.py              `-- wishlist.py
|   |   |-- repositories/            # DB query layer
|   |   |   |-- booking_repository.py  |-- hotel_repository.py
|   |   |   |-- notification_repository.py
|   |   |   |-- payment_repository.py  |-- pricing_repository.py
|   |   |   |-- review_repository.py   |-- room_repository.py
|   |   |   `-- user_repository.py
|   |   |-- schemas/                 # Pydantic v2 request/response models
|   |   |   |-- auth.py  |-- booking.py  |-- hotel.py  |-- payment.py
|   |   |   |-- pricing.py  |-- review.py  |-- room.py  |-- user.py
|   |   |-- services/                # business logic layer
|   |   |   |-- auth.py              # Google OAuth, email verify, password reset
|   |   |   |-- availability.py      # room availability query (date-range JOIN)
|   |   |   |-- cache.py             # Redis availability + generic cache (R6)
|   |   |   |-- email.py             # SMTP / Celery email dispatch
|   |   |   |-- pricing.py           # dynamic pricing rule resolution
|   |   |   |-- reports.py           # occupancy and revenue aggregations
|   |   |   |-- search.py            # Meilisearch client + ILIKE fallback (R5)
|   |   |   |-- storage.py           # MinIO pre-signed URL helpers
|   |   |   |-- token_blacklist.py   # JWT blacklist via Redis
|   |   |   `-- websocket.py         # ConnectionManager + Redis Pub/Sub (R7)
|   |   |-- tasks/                   # Celery tasks (R10)
|   |   |   |-- batch_tasks.py       # nightly revenue snapshot, hourly warmup
|   |   |   |-- email_tasks.py       # async email delivery
|   |   |   |-- indexing_tasks.py    # Meilisearch re-index on hotel/room change
|   |   |   `-- payment_tasks.py     # async payment intent processing
|   |   |-- utils/
|   |   |   |-- helpers.py  |-- logger.py  |-- seed_data.py  |-- validators.py
|   |   |-- celery_app.py            # Celery app + Beat schedule config
|   |   |-- dependencies.py          # FastAPI Depends: get_db, get_current_user
|   |   `-- main.py                  # FastAPI app, middleware chain, router mount
|   |-- migrations/                  # Alembic (R3)
|   |   |-- versions/
|   |   |   |-- 20260415_0001_create_hotel_tables.py
|   |   |   |-- 20260427_0002_add_guest_count_cancellation.py
|   |   |   |-- 20260428_0003_add_photo_url_to_users.py
|   |   |   |-- 20260428_0004_assign_hotel_to_admin_staff.py
|   |   |   |-- 20260428_0005_partition_bookings.py
|   |   |   |-- 20260428_0006_auth_email_verify_password_reset.py
|   |   |   `-- 20260429_0007_add_room_id_to_wishlist.py
|   |   `-- env.py
|   |-- scripts/
|   |   |-- meilisearch_seed.py      # one-time Meilisearch index seed (R5)
|   |   |-- migrate_avatars_to_minio.py
|   |   `-- migrate_hotel_images_to_minio.py
|   |-- tests/                       # pytest integration suite
|   |   |-- conftest.py  |-- factories.py
|   |   |-- test_admin.py  |-- test_auth.py
|   |   |-- test_bookings.py  |-- test_pricing.py
|   |   `-- test_rooms.py
|   |-- requirements.txt
|   `-- alembic.ini
|-- frontend/react-app/src/
|   |-- components/
|   |   |-- admin/                   # admin-specific UI widgets
|   |   |-- common/                  # Toast, Navbar, Footer, etc.
|   |   |-- public/                  # AIHelper concierge chat
|   |   |-- staff/                   # staff dashboard widgets
|   |   `-- user/                    # NotificationItem, BookingCard, etc.
|   |-- contexts/
|   |   |-- AuthContext.jsx          # JWT token, user profile, login/logout
|   |   |-- NotificationContext.jsx  # real-time toast state (R7)
|   |   |-- SocketContext.jsx        # WebSocket connection lifecycle (R7)
|   |   |-- ThemeContext.jsx
|   |   `-- WishlistContext.jsx
|   |-- pages/
|   |   |-- admin/   |-- auth/   |-- public/   |-- staff/   |-- user/
|   |-- services/
|   |   |-- api.js                   # Axios instance with JWT interceptor
|   |   |-- auth.js  |-- user.js  |-- admin.js
|   |-- locales/                     # i18next translations (11 languages)
|   |   |-- en/  ar/  de/  es/  fr/  it/  ja/  ko/  pt/  ru/  zh/
|   |-- utils/
|   |   |-- i18n.js                  # i18next initialisation
|   |-- App.jsx  |-- main.jsx        # root component + Vite entry point
|-- docker/
|   |-- Dockerfile.backend           # Python 3.11-slim, gunicorn + uvicorn
|   |-- Dockerfile.celery            # same base, celery entrypoint
|   |-- Dockerfile.frontend          # node:20 build -> nginx:alpine serve
|   |-- deploy.sh                    # Docker Swarm stack deploy script
|   |-- traefik.yml                  # Traefik static config (R8)
|   |-- traefik-dynamic.yml          # middlewares, circuit breaker (R8)
|   |-- nginx-static.conf            # nginx config for React SPA
|   `-- pgbouncer.ini                # PgBouncer connection pool config
|-- .github/workflows/
|   |-- deploy-backend.yml           # build + push GHCR + swarm deploy (R13)
|   |-- deploy-frontend.yml          # npm build + rsync dist/ (R13)
|   |-- certbot.yml                  # monthly TLS cert renewal
|   `-- reset-credentials.yml        # utility: reset admin credentials
|-- docker-compose.yml               # 21-service local dev stack (R9)
|-- docker-stack.yml                 # production Swarm stack (8 services)
`-- README.md
\end{lstlisting}
...
\textbf{Endpoint:} \texttt{wss://\{host\}/ws/\{user\_id\}?token=\{jwt\}}
...
\textbf{Cross-replica fan-out:} Because two backend replicas share the same Redis instance, each replica publishes notification events to a Redis Pub/Sub channel (\texttt{ws:events}). Every replica subscribes and fans out to locally-connected WebSocket clients, ensuring a guest receives notifications regardless of which replica Traefik routed their HTTP upgrade to.

\textbf{React frontend client:} Three React modules implement the client side:
\begin{itemize}[noitemsep]
  \item \texttt{contexts/SocketContext.jsx} --- opens \texttt{wss://allstay.rest/ws/\{user\_id\}?token=\{jwt\}} on login; automatically reconnects after 3 seconds if the connection drops.
  \item \texttt{contexts/NotificationContext.jsx} --- listens to \texttt{SocketContext} and queues incoming events (\texttt{booking\_update}, \texttt{notification}) as timed toasts (4.5-second auto-dismiss).
  \item \texttt{components/common/Toast.jsx} --- renders a fixed-position toast stack in the bottom-right corner of every page; click to dismiss early.
\end{itemize}

\textbf{Sample server$\to$client payload:}
\begin{lstlisting}[language={}]
{"event": "booking_update", "data": {"booking_id": 42, "status": "confirmed"}}
{"event": "notification",   "data": {"title": "Check-in ready", "message": "..."}}
{"event": "ping",           "data": {}}
\end{lstlisting}
...
Four distinct storage engines are used, each chosen for its data model strengths:
...
\midrule
PostgreSQL~15  & Relational    & Transactional consistency, FK integrity, ACID guarantees for bookings and payments. Primary handles all writes; hot-standby replica serves analytics read queries. \\[3pt]
Redis~7        & Key-value     & Sub-millisecond reads for availability cache (5-min TTL); atomic Lua scripts for token-bucket rate-limiter state; Pub/Sub for WebSocket cross-replica fan-out; list-based Celery task queue. \\[3pt]
Meilisearch    & Search index  & Full-text hotel and room search with typo tolerance and faceted filters. PostgreSQL \texttt{ILIKE} scans cannot rank by relevance and degrade on large datasets. Deployed as a Docker Swarm service (1/1 replica); index is seeded via \texttt{scripts/meilisearch\_seed.py} and kept fresh by \texttt{indexing\_tasks.py} on hotel/room mutations. The search endpoint falls back to PostgreSQL \texttt{ILIKE} automatically when Meilisearch is unreachable. \\[3pt]
MinIO          & Object (S3)   & Binary large objects (guest avatars, hotel photos) are stored as S3-compatible objects. MinIO scales independently, avoids DB bloat, and exposes pre-signed URLs for direct browser download without proxying through the API. \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Availability cache:} When a guest searches for available rooms for a specific (hotel, check\_in, check\_out) triple, the result is written to Redis with a 5-minute TTL. Key: \texttt{avail:\{hotel\_id\}:\{ci\}:\{co\}}. On cache hit the handler returns immediately; on miss it queries PostgreSQL and populates the cache (\texttt{services/cache.py}).

\textbf{Cache invalidation:} When a booking is created or cancelled, \texttt{invalidate\_availability(hotel\_id, check\_in, check\_out)} is called immediately after the database write. The function deletes the exact cache key plus $\pm$1-day overlap keys to handle bookings that span multiple cached windows. This prevents stale availability data from being served after a room becomes unavailable.
...
\begin{itemize}[noitemsep]
  ...
  \item \texttt{hms-db} (164.92.193.226) --- PostgreSQL~15 primary + replica (private network only, no public exposure).
\end{itemize}

GitHub Actions workflows (\texttt{.github/workflows/}) deploy on every push to \texttt{main}: backend changes build and push Docker images to GHCR, then run \texttt{docker stack deploy} via SSH to perform a rolling update of the Swarm services; frontend changes run \texttt{npm run build} + rsync the \texttt{dist/} bundle to \texttt{/opt/hms/frontend} on the server.
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{WebSocket reconnection:} The React \texttt{SocketContext} auto-reconnects after 3 seconds on disconnect. However, events published during the disconnection window are not replayed --- the client misses any notifications sent while offline.
  ...
  \item \textbf{Meilisearch initial seed:} Meilisearch is deployed and running in production (1/1 replica). The \texttt{indexing\_tasks} Celery task keeps the index fresh on hotel/room mutations. The one-time initial bulk seed (\texttt{scripts/meilisearch\_seed.py}) is pending; until it completes, the search endpoint falls back to PostgreSQL \texttt{ILIKE}.
...
%  12. DEFAULT ACCOUNTS
% ══════════════════════════════════════════════════════════════════════════════
\section{Default Accounts (after seeding)}
...
\vspace{2em}
\noindent
\textit{Note:} Due to workflow convenience during development, the project repository activity appears under a single GitHub account. However, the project tasks and implementation work were completed collaboratively by all team members according to the distributed responsibilities.
